...
-- 3. Create risk assessment function
...
WITH dispute_stats AS (
    SELECT 
        ...
        COUNT(*) FILTER (WHERE dispute_status = 'UNDER_INVESTIGATION') as investigating_disputes,
        COUNT(*) FILTER (WHERE dispute_status IN ('MEDIATION', 'ARBITRATION', 'LITIGATION')) as active_legal_disputes,
        COUNT(*) FILTER (WHERE dispute_status = 'RESOLVED') as resolved_disputes,
        SUM(disputed_amount) as total_disputed_amount,
        SUM(claimed_damages) as total_claimed_damages,
        SUM(settlement_amount) as total_settlement_amount,
        AVG(disputed_amount) FILTER (WHERE disputed_amount > 0) as avg_disputed_amount,
        COUNT(*) FILTER (WHERE dispute_type = 'RENT_DISPUTE') as rent_disputes,
        COUNT(*) FILTER (WHERE dispute_type = 'DEPOSIT_DISPUTE') as deposit_disputes,
        COUNT(*) FILTER (WHERE dispute_type = 'MAINTENANCE_DISPUTE') as maintenance_disputes,
        COUNT(*) FILTER (WHERE dispute_type = 'EVICTION') as eviction_cases,
        COUNT(*) FILTER (WHERE DATE_TRUNC('month', created_at) = DATE_TRUNC('month', CURRENT_DATE)) as this_month_disputes,
...
risk_stats AS (
    SELECT 
        ...
        COUNT(*) FILTER (WHERE assessment_status = 'ACTIVE') as active_risks,
        COUNT(*) FILTER (WHERE DATE_TRUNC('month', created_at) = DATE_TRUNC('month', CURRENT_DATE)) as this_month_assessments,
        MAX(updated_at) as last_risk_update
    FROM bms.risk_assessments
    GROUP BY company_id
),
compliance_stats AS (
    SELECT 
        ...
        COUNT(*) FILTER (WHERE compliance_status = 'NON_COMPLIANT') as non_compliant_requirements,
        COUNT(*) FILTER (WHERE compliance_deadline < CURRENT_DATE AND compliance_status != 'COMPLIANT') as overdue_requirements,
        MAX(updated_at) as last_compliance_update
    FROM bms.legal_compliance_requirements
    GROUP BY company_id
),
insurance_stats AS (
    SELECT 
        company_id,
        COUNT(*) as total_insurance_policies,
        COUNT(*) FILTER (WHERE policy_status = 'ACTIVE') as active_policies,
        COUNT(*) FILTER (WHERE policy_end_date <= CURRENT_DATE + INTERVAL '30 days') as expiring_soon_policies,
        SUM(coverage_amount) as total_coverage_amount,
        SUM(annual_premium) as total_annual_premiums
    FROM bms.insurance_policies
    GROUP BY company_id
)
SELECT 
    COALESCE(ds.company_id, rs.company_id, cs.company_id, ins.company_id) as company_id,
    COALESCE(ds.total_disputes, 0) as total_disputes,
    COALESCE(ds.open_disputes, 0) as open_disputes,
    COALESCE(ds.investigating_disputes, 0) as investigating_disputes,
    COALESCE(ds.active_legal_disputes, 0) as active_legal_disputes,
    COALESCE(ds.resolved_disputes, 0) as resolved_disputes,
    COALESCE(ds.total_disputed_amount, 0) as total_disputed_amount,
    COALESCE(ds.total_claimed_damages, 0) as total_claimed_damages,
    COALESCE(ds.total_settlement_amount, 0) as total_settlement_amount,
    ds.avg_disputed_amount,
    COALESCE(ds.rent_disputes, 0) as rent_disputes,
    COALESCE(ds.deposit_disputes, 0) as deposit_disputes,
    COALESCE(ds.maintenance_disputes, 0) as maintenance_disputes,
    COALESCE(ds.eviction_cases, 0) as eviction_cases,
    COALESCE(rs.total_risk_assessments, 0) as total_risk_assessments,
    COALESCE(rs.critical_risks, 0) as critical_risks,
    COALESCE(rs.high_risks, 0) as high_risks,
    COALESCE(rs.active_risks, 0) as active_risks,
    COALESCE(cs.total_compliance_requirements, 0) as total_compliance_requirements,
    COALESCE(cs.non_compliant_requirements, 0) as non_compliant_requirements,
    COALESCE(cs.overdue_requirements, 0) as overdue_requirements,
    COALESCE(ins.total_insurance_policies, 0) as total_insurance_policies,
    COALESCE(ins.active_policies, 0) as active_policies,
    COALESCE(ins.expiring_soon_policies, 0) as expiring_soon_policies,
    COALESCE(ins.total_coverage_amount, 0) as total_coverage_amount,
    COALESCE(ins.total_annual_premiums, 0) as total_annual_premiums,
    COALESCE(ds.this_month_disputes, 0) as this_month_disputes,
    COALESCE(rs.this_month_assessments, 0) as this_month_assessments,
    ds.last_dispute_update,
    rs.last_risk_update,
    cs.last_compliance_update
FROM dispute_stats ds
FULL OUTER JOIN risk_stats rs ON ds.company_id = rs.company_id
FULL OUTER JOIN compliance_stats cs ON COALESCE(ds.company_id, rs.company_id) = cs.company_id
FULL OUTER JOIN insurance_stats ins ON COALESCE(ds.company_id, rs.company_id, cs.company_id) = ins.company_id;-- 7.
 Dispute case details view
