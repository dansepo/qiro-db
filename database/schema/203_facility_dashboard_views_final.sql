-- =====================================================
-- 시설 관리 대시보드 뷰 최종 수정
-- Phase 10: Dashboard Views Final Fix
-- =====================================================

-- 1. 실시간 현황 대시보드 뷰 최종 수정
...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN pme.execution_date <= CURRENT_DATE AND pme.execution_status = 'PENDING' THEN mp.plan_id END) as due_maintenance,
    COUNT(DISTINCT CASE WHEN pme.execution_date <= CURRENT_DATE + INTERVAL '7 days' AND pme.execution_status = 'PENDING' THEN mp.plan_id END) as upcoming_maintenance,
    
    -- 비용 현황 (이번 달) - work_cost_settlements 테이블 사용
    COALESCE(SUM(CASE WHEN wcs.settlement_date >= DATE_TRUNC('month', CURRENT_DATE) THEN wcs.total_actual_cost END), 0) as monthly_cost,
    COALESCE(SUM(CASE WHEN wcs.settlement_date >= DATE_TRUNC('year', CURRENT_DATE) THEN wcs.total_actual_cost END), 0) as yearly_cost,
...
SELECT 
    ...
    -- 평균 해결 시간 (시간 단위) - resolved_at과 reported_at 사용
    ...
    COALESCE(SUM(wcs.total_actual_cost), 0) + COALESCE(SUM(wolc.total_labor_cost), 0) as total_maintenance_cost,
    COALESCE(SUM(wolc.total_labor_cost), 0) as labor_cost,
    COALESCE(SUM(wcs.actual_material_cost), 0) as material_cost,
    COALESCE(SUM(wcs.contractor_fees), 0) as contractor_cost,
    ...
    -- 예방 정비 통계
    COUNT(DISTINCT mp.plan_id) as total_maintenance_plans,
    COUNT(DISTINCT CASE WHEN pme.execution_date >= DATE_TRUNC('month', CURRENT_DATE) 
        AND pme.execution_date < DATE_TRUNC('month', CURRENT_DATE) + INTERVAL '1 month' 
        AND pme.execution_status = 'COMPLETED' THEN mp.plan_id END) as completed_maintenance,
    COUNT(DISTINCT CASE WHEN pme.execution_date < CURRENT_DATE 
        AND pme.execution_status = 'PENDING' THEN mp.plan_id END) as overdue_maintenance,
...
SELECT 
    ...
    COALESCE(SUM(CASE WHEN wcs.settlement_date >= CURRENT_DATE - INTERVAL '7 days' THEN wcs.total_actual_cost END), 0) as weekly_cost,
    COALESCE(SUM(CASE WHEN wcs.settlement_date >= DATE_TRUNC('month', CURRENT_DATE) THEN wcs.total_actual_cost END), 0) as monthly_cost,
    COALESCE(SUM(CASE WHEN wcs.settlement_date >= DATE_TRUNC('quarter', CURRENT_DATE) THEN wcs.total_actual_cost END), 0) as quarterly_cost,
    COALESCE(SUM(CASE WHEN wcs.settlement_date >= DATE_TRUNC('year', CURRENT_DATE) THEN wcs.total_actual_cost END), 0) as yearly_cost,
    
    -- 비용 유형별 분석
    COALESCE(SUM(wolc.total_labor_cost), 0) as total_labor_cost,
    COALESCE(SUM(wcs.actual_material_cost), 0) as total_material_cost,
    COALESCE(SUM(wcs.contractor_fees), 0) as total_contractor_cost,
    COALESCE(SUM(wcs.actual_equipment_cost), 0) as total_equipment_cost,
    
    -- 작업 카테고리별 비용
    COALESCE(SUM(CASE WHEN wo.work_category = 'PREVENTIVE' THEN wcs.total_actual_cost END), 0) as preventive_maintenance_cost,
    COALESCE(SUM(CASE WHEN wo.work_category = 'CORRECTIVE' THEN wcs.total_actual_cost END), 0) as corrective_maintenance_cost,
    COALESCE(SUM(CASE WHEN wo.work_category = 'EMERGENCY' THEN wcs.total_actual_cost END), 0) as emergency_repair_cost,
    
    -- 시설물 유형별 비용
    COALESCE(SUM(CASE WHEN fa.asset_type = 'ELECTRICAL' THEN wcs.total_actual_cost END), 0) as electrical_cost,
    COALESCE(SUM(CASE WHEN fa.asset_type = 'PLUMBING' THEN wcs.total_actual_cost END), 0) as plumbing_cost,
    COALESCE(SUM(CASE WHEN fa.asset_type = 'HVAC' THEN wcs.total_actual_cost END), 0) as hvac_cost,
    COALESCE(SUM(CASE WHEN fa.asset_type = 'ELEVATOR' THEN wcs.total_actual_cost END), 0) as elevator_cost,
    COALESCE(SUM(CASE WHEN fa.asset_type = 'FIRE_SAFETY' THEN wcs.total_actual_cost END), 0) as fire_safety_cost,
    
    -- 평균 비용 분석
    COALESCE(AVG(wcs.total_actual_cost), 0) as avg_cost_per_work_order,
    COALESCE(COUNT(DISTINCT wcs.work_order_id), 0) as total_cost_entries,
    
    -- 최고/최저 비용
    COALESCE(MAX(wcs.total_actual_cost), 0) as highest_single_cost,
    COALESCE(MIN(wcs.total_actual_cost), 0) as lowest_single_cost,
...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN pme.execution_date < CURRENT_DATE AND pme.execution_status = 'PENDING' THEN mp.plan_id END) as overdue_maintenance,
    ...
    -- 예산 초과 위험
    CASE 
        WHEN SUM(wcs.total_actual_cost) > 100000 THEN 'HIGH'
        WHEN SUM(wcs.total_actual_cost) > 50000 THEN 'MEDIUM'
        ELSE 'LOW'
...
-- 5. 간단한 대시보드 KPI 뷰 생성
CREATE OR REPLACE VIEW bms.v_facility_dashboard_kpi AS
SELECT 
    c.company_id,
    c.company_name,
    ...
    COUNT(DISTINCT fa.asset_id) as total_assets,
    COUNT(DISTINCT CASE WHEN fa.asset_status = 'ACTIVE' THEN fa.asset_id END) as active_assets,
    ROUND(COALESCE(AVG(fa.uptime_percentage), 0), 2) as avg_uptime_percentage,
    COUNT(DISTINCT fr.report_id) as total_reports_this_month,
    COUNT(DISTINCT wo.work_order_id) as total_work_orders_this_month,
    COALESCE(SUM(wcs.total_actual_cost), 0) as total_cost_this_month,
    
    -- 상태별 시설물 수
    COUNT(DISTINCT CASE WHEN fa.condition_rating = 'EXCELLENT' THEN fa.asset_id END) as excellent_assets,
    COUNT(DISTINCT CASE WHEN fa.condition_rating = 'GOOD' THEN fa.asset_id END) as good_assets,
    COUNT(DISTINCT CASE WHEN fa.condition_rating = 'FAIR' THEN fa.asset_id END) as fair_assets,
    COUNT(DISTINCT CASE WHEN fa.condition_rating = 'POOR' THEN fa.asset_id END) as poor_assets,
...
-- 6. 뷰 코멘트 추가
COMMENT ON VIEW bms.v_facility_dashboard_overview IS '시설 관리 실시간 현황 대시보드 뷰 (최종)';
COMMENT ON VIEW bms.v_monthly_facility_report IS '월별 시설 관리 현황 보고서 뷰 (최종)';
COMMENT ON VIEW bms.v_cost_analysis_report IS '비용 분석 및 통계 보고서 뷰 (최종)';
COMMENT ON VIEW bms.v_facility_alerts_dashboard IS '시설 관리 알림 및 경고 대시보드 뷰 (최종)';
COMMENT ON VIEW bms.v_facility_dashboard_kpi IS '시설 관리 대시보드 KPI 뷰';