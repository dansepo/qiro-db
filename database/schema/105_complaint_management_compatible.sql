-- =====================================================
-- 민원 및 회계 관리 시스템 - 민원 관리 테이블 (기존 구조 호환)
-- 작성일: 2025-01-30
...
CREATE TABLE IF NOT EXISTS bms.complaints (
    ...
    unit_id UUID REFERENCES bms.units(unit_id),
    complaint_category_id BIGINT NOT NULL REFERENCES bms.complaint_categories(category_id),
    
    -- 신고자 정보
    reporter_id UUID REFERENCES bms.users(user_id),
    ...
    -- 담당자 배정
    assigned_to UUID REFERENCES bms.users(user_id),
...
CREATE TABLE IF NOT EXISTS bms.complaint_communications (
    ...
    complaint_id BIGINT NOT NULL REFERENCES bms.complaints(complaint_id) ON DELETE CASCADE,
    sender_id UUID REFERENCES bms.users(user_id),
    sender_name VARCHAR(255) NOT NULL,
    receiver_id UUID REFERENCES bms.users(user_id),
...
CREATE TABLE IF NOT EXISTS bms.complaint_status_history (
    ...
    previous_assignee UUID REFERENCES bms.users(user_id),
    new_assignee UUID REFERENCES bms.users(user_id),
    comment TEXT,
    action_by UUID NOT NULL REFERENCES bms.users(user_id),
...
-- 기본 민원 분류 데이터 삽입 함수
CREATE OR REPLACE FUNCTION bms.insert_default_complaint_categories(p_company_id UUID)
RETURNS VOID AS $$
BEGIN
