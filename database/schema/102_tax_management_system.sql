...
CREATE TABLE IF NOT EXISTS bms.vat_returns (
    ...
    filing_status filing_status DEFAULT 'NOT_FILED',
    filed_at TIMESTAMP,
    filed_by BIGINT REFERENCES bms.users(id),
...
CREATE TABLE IF NOT EXISTS bms.tax_filing_schedule (
    ...
    completed_by BIGINT REFERENCES bms.users(id),
...
CREATE TABLE IF NOT EXISTS bms.tax_validation_log (
    ...
    validated_by BIGINT REFERENCES bms.users(id)
);
...
-- 부가세 신고 데이터 계산 함수
CREATE OR REPLACE FUNCTION bms.calculate_vat_return_data(
    p_company_id BIGINT,
...
DECLARE
    ...
    v_output_tax DECIMAL(15,2) := 0;
    v_total_purchase DECIMAL(15,2) := 0;
    v_input_tax DECIMAL(15,2) := 0;
    v_net_tax DECIMAL(15,2) := 0;
BEGIN
    -- 해당 기간의 부가세 신고서 조회
    SELECT return_id INTO v_return_id
    FROM bms.vat_returns
    WHERE company_id = p_company_id 
      AND filing_period = p_filing_period
      AND return_type = 'REGULAR';
    
    -- 매출 관련 집계
    SELECT 
        COALESCE(SUM(supply_amount + tax_amount), 0),
        COALESCE(SUM(supply_amount), 0),
        COALESCE(SUM(tax_amount), 0)
    INTO v_total_sales, v_taxable_sales, v_output_tax
    FROM bms.tax_transactions
    WHERE company_id = p_company_id
      AND filing_period = p_filing_period
    ...
    SELECT 
        COALESCE(SUM(supply_amount + tax_amount), 0),
        COALESCE(SUM(CASE WHEN is_deductible THEN tax_amount ELSE 0 END), 0)
    INTO v_total_purchase, v_input_tax
    FROM bms.tax_transactions
    WHERE company_id = p_company_id
      AND filing_period = p_filing_period
    ...
    -- 납부할 세액 계산
    v_net_tax := v_output_tax - v_input_tax;
    
    -- 부가세 신고서 업데이트
    UPDATE bms.vat_returns
    SET 
        total_sales_amount = v_total_sales,
        taxable_sales_amount = v_taxable_sales,
        output_tax_amount = v_output_tax,
        total_purchase_amount = v_total_purchase,
        deductible_purchase_amount = v_total_purchase,
        input_tax_amount = v_input_tax,
        net_tax_amount = v_net_tax,
...
-- 세무 신고 일정 자동 생성 함수
CREATE OR REPLACE FUNCTION bms.create_tax_filing_schedule(
    p_company_id BIGINT,
    p_year INTEGER
)
RETURNS VOID AS $$
DECLARE
    v_month INTEGER;
    v_filing_period VARCHAR(7);
    v_due_date DATE;
    v_filing_deadline DATE;
BEGIN
    -- 월별 부가세 신고 일정 생성
    FOR v_month IN 1..12 LOOP
        v_filing_period := p_year || '-' || LPAD(v_month::TEXT, 2, '0');
        
        -- 신고 기한: 다음 달 25일
        v_due_date := DATE(p_year || '-' || LPAD(v_month::TEXT, 2, '0') || '-01') + INTERVAL '1 month' + INTERVAL '24 days';
        v_filing_deadline := v_due_date;
        
        -- 부가세 신고 일정 생성
        INSERT INTO bms.tax_filing_schedule (
            company_id, tax_type, filing_period, filing_year, filing_month,
            due_date, filing_deadline
        ) VALUES (
            p_company_id, 'VAT', v_filing_period, p_year, v_month,
            v_due_date, v_filing_deadline
        ) ON CONFLICT (company_id, tax_type, filing_period) DO NOTHING;
    END LOOP;
    ...
    FOR v_month IN 1..12 LOOP
        ...
        -- 신고 기한: 다음 달 10일
        v_due_date := DATE(p_year || '-' || LPAD(v_month::TEXT, 2, '0') || '-01') + INTERVAL '1 month' + INTERVAL '9 days';
        v_filing_deadline := v_due_date;
        ...
        INSERT INTO bms.tax_filing_schedule (
            ...
            p_company_id, 'WITHHOLDING_TAX', v_filing_period, p_year, v_month,
            v_due_date, v_filing_deadline
        ) ON CONFLICT (company_id, tax_type, filing_period) DO NOTHING;
    END LOOP;
END;
...
-- 세무 데이터 검증 함수
CREATE OR REPLACE FUNCTION bms.validate_tax_data(
...
BEGIN
    -- 세무 거래 검증
    IF p_entity_type = 'TAX_TRANSACTION' THEN
        SELECT company_id INTO v_company_id
        FROM bms.tax_transactions
        WHERE transaction_id = p_entity_id;
        
        -- 금액 검증
        IF EXISTS (
            SELECT 1 FROM bms.tax_transactions
            WHERE transaction_id = p_entity_id
              AND (supply_amount < 0 OR tax_amount < 0 OR total_amount != supply_amount + tax_amount)
        ) THEN
            v_is_valid := false;
            v_error_message := '금액 정보가 올바르지 않습니다.';
        END IF;
        ...
        IF EXISTS (
            ...
            v_is_valid := false;
            v_error_message := '세금계산서 번호가 중복됩니다.';
        END IF;
    END IF;
    
    -- 검증 로그 기록
    INSERT INTO bms.tax_validation_log (
        company_id, entity_type, entity_id, validation_type,
        is_valid, error_message, validated_at
    ) VALUES (
        v_company_id, p_entity_type, p_entity_id, 'DATA_VALIDATION',
        v_is_valid, v_error_message, CURRENT_TIMESTAMP
    );
...
-- 기본 세무 계산 규칙 삽입 함수
CREATE OR REPLACE FUNCTION bms.insert_default_tax_calculation_rules(p_company_id BIGINT)
RETURNS VOID AS $$
BEGIN
    -- 부가세 계산 규칙
