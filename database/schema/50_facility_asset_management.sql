...
CREATE TABLE IF NOT EXISTS bms.facility_assets (
    ...
    CONSTRAINT chk_dates CHECK (
        (warranty_end_date IS NULL OR warranty_start_date IS NULL OR warranty_end_date >= warranty_start_date) AND
        (next_inspection_date IS NULL OR last_inspection_date IS NULL OR next_inspection_date > last_inspection_date) AND
        (next_maintenance_date IS NULL OR last_maintenance_date IS NULL OR next_maintenance_date > last_maintenance_date)
    )
);-- 3
...
CREATE INDEX IF NOT EXISTS idx_facility_categories_level ON bms.facility_categories(category_level);
...
CREATE INDEX IF NOT EXISTS idx_facility_assets_manufacturer ON bms.facility_assets(manufacturer);
CREATE INDEX IF NOT EXISTS idx_facility_assets_installation_date ON bms.facility_assets(installation_date);
...
CREATE INDEX IF NOT EXISTS idx_facility_assets_technician ON bms.facility_assets(assigned_technician_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_status_history_reason ON bms.asset_status_history(change_reason);
CREATE INDEX IF NOT EXISTS idx_asset_status_history_changed_by ON bms.asset_status_history(changed_by);
...
CREATE INDEX IF NOT EXISTS idx_asset_depreciation_period ON bms.asset_depreciation(depreciation_year, depreciation_month);
...
CREATE INDEX IF NOT EXISTS idx_asset_documents_status ON bms.asset_documents(document_status);
CREATE INDEX IF NOT EXISTS idx_asset_documents_expiry ON bms.asset_documents(expiry_date);

-- Composite indexes for better query performance
CREATE INDEX IF NOT EXISTS idx_facility_assets_company_status ON bms.facility_assets(company_id, asset_status);
CREATE INDEX IF NOT EXISTS idx_facility_assets_company_type ON bms.facility_assets(company_id, asset_type);
CREATE INDEX IF NOT EXISTS idx_facility_assets_building_status ON bms.facility_assets(building_id, asset_status);
CREATE INDEX IF NOT EXISTS idx_asset_status_history_asset_date ON bms.asset_status_history(asset_id, change_date);
