...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN mte.scheduled_date <= CURRENT_DATE THEN mp.plan_id END) as due_maintenance,
    COUNT(DISTINCT CASE WHEN mte.scheduled_date <= CURRENT_DATE + INTERVAL '7 days' THEN mp.plan_id END) as upcoming_maintenance,
...
LEFT JOIN bms.maintenance_task_executions mte ON mp.plan_id = mte.plan_id
...
SELECT 
    ...
    -- 예방 정비 통계
    COUNT(DISTINCT mp.plan_id) as total_maintenance_plans,
    COUNT(DISTINCT CASE WHEN mte.execution_date >= DATE_TRUNC('month', CURRENT_DATE) THEN mte.execution_id END) as completed_maintenance,
    COUNT(DISTINCT CASE WHEN mte.scheduled_date < CURRENT_DATE AND mte.execution_status != 'COMPLETED' THEN mp.plan_id END) as overdue_maintenance,
