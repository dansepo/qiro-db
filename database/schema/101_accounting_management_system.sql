...
CREATE TABLE IF NOT EXISTS bms.accounting_entries (
    ...
    approved_by BIGINT REFERENCES bms.users(id),
    approved_at TIMESTAMP,
    approval_comment TEXT,
    
    -- 회계 처리 상태
    entry_status entry_status NOT NULL DEFAULT 'DRAFT',
    posted_by BIGINT REFERENCES bms.users(id),
    posted_at TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS bms.financial_reports (
    ...
    finalized_by BIGINT REFERENCES bms.users(id),
...
CREATE TABLE IF NOT EXISTS bms.approval_requests (
    ...
    current_approver BIGINT REFERENCES bms.users(id),
    
    -- 승인 상태
    approval_status approval_status NOT NULL DEFAULT 'PENDING',
    final_approved_by BIGINT REFERENCES bms.users(id),
    final_approved_at TIMESTAMP,
    
    -- 요청자 정보
    requested_by BIGINT NOT NULL REFERENCES bms.users(id),
...
CREATE TABLE IF NOT EXISTS bms.approval_history (
    ...
    approver_id BIGINT NOT NULL REFERENCES bms.users(id),
    ...
    -- 위임 정보 (DELEGATED인 경우)
    delegated_to BIGINT REFERENCES bms.users(id),
...
-- 월별 수입/지출 요약 뷰
CREATE OR REPLACE VIEW bms.v_monthly_financial_summary AS
SELECT 
    ae.company_id,
    ae.building_id,
    ae.fiscal_year,
    ae.fiscal_month,
    ac.account_type,
    SUM(CASE WHEN ae.entry_type = 'REVENUE' THEN ae.net_amount ELSE 0 END) as total_revenue,
    SUM(CASE WHEN ae.entry_type = 'EXPENSE' THEN ae.net_amount ELSE 0 END) as total_expense,
    SUM(CASE WHEN ae.entry_type = 'REVENUE' THEN ae.net_amount ELSE -ae.net_amount END) as net_amount,
    COUNT(*) as transaction_count
FROM bms.accounting_entries ae
JOIN bms.account_codes ac ON ae.account_code_id = ac.account_id
WHERE ae.entry_status = 'POSTED'
GROUP BY ae.company_id, ae.building_id, ae.fiscal_year, ae.fiscal_month, ac.account_type;

-- 예산 대비 실적 분석 뷰
CREATE OR REPLACE VIEW bms.v_budget_vs_actual AS
SELECT 
    bp.company_id,
    bp.building_id,
    bp.fiscal_year,
    ...
    CASE 
        WHEN bp.planned_amount > 0 THEN 
            ROUND((bp.executed_amount / bp.planned_amount * 100), 2)
        ELSE 0 
    END as actual_execution_rate,
    CASE 
        WHEN bp.executed_amount > bp.planned_amount THEN 'OVER_BUDGET'
        WHEN bp.execution_rate >= 90 THEN 'NEAR_BUDGET'
        WHEN bp.execution_rate >= 50 THEN 'ON_TRACK'
        ELSE 'UNDER_BUDGET'
    END as budget_status_indicator
FROM bms.budget_plans bp
JOIN bms.account_codes ac ON bp.account_code_id = ac.account_id
WHERE bp.budget_status = 'ACTIVE';

-- 승인 대기 현황 뷰
CREATE OR REPLACE VIEW bms.v_pending_approvals AS
SELECT 
    ar.request_id,
    ar.workflow_id,
    aw.workflow_name,
    ...
    ar.request_amount,
    ar.current_level,
    u1.name as current_approver_name,
    u2.name as requested_by_name,
    ar.requested_at,
    EXTRACT(DAYS FROM CURRENT_TIMESTAMP - ar.requested_at) as pending_days
FROM bms.approval_requests ar
JOIN bms.approval_workflows aw ON ar.workflow_id = aw.workflow_id
LEFT JOIN bms.users u1 ON ar.current_approver = u1.id
LEFT JOIN bms.users u2 ON ar.requested_by = u2.id
...
-- 기본 계정과목 데이터 삽입 함수
CREATE OR REPLACE FUNCTION bms.insert_default_account_codes(p_company_id BIGINT)
RETURNS VOID AS $$
BEGIN
...
-- 기본 거래 분류 데이터 삽입 함수
CREATE OR REPLACE FUNCTION bms.insert_default_transaction_categories(p_company_id BIGINT)
RETURNS VOID AS $$
BEGIN
